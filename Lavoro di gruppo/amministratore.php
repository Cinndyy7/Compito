...
<?php
    ...
    if(!isset($_SESSION['user_id'])){
        echo '<script>alert("Non ti sei loggato"); window.location.href = "Donne.php";</script>';     
    }
...
<body>
    ...
    <main>
        <p>Gli admin possono aggiungere/modificare/eliminare donne nel database o caricare presentazioni. </p>
        <?php
            ...
            if($result && $result->num_rows > 0) {
                ...
                while($row = $result->fetch_assoc()) {
                    ...
                    echo '<button class="button" type="button" onclick="location.href=\'donna_delete.php?ID=' . $row['ID'] . '\'">Elimina</button>';
                    echo '<button class="button" type="button" onclick="location.href=\'donna_edit.php?ID=' . $row['ID'] . '\'">Modifica</button>';
                    echo '</div></div>';
                    echo '</div><br>';
                }
                echo '<button class="button" type="button" onclick="location.href=\'donna_insert.php\'">Aggiungi</button>';
            } else {
            ...
            if($result && $result->num_rows > 0) {
                echo '<h3>Elenco utenti</h3>';
                while($row = $result->fetch_assoc()) {
                    echo '<div class="card">';
                    echo '<h3>' . $row['NOME'] . ' ' . $row['COGNOME'] . '</h3>';
                    echo '<button class="donna-button" type="button" onclick="location.href=\'profilo.php?ID=' . $row['ID'] . '\'">Visualizza</button>';

                    if($row["ID"] != $_SESSION["user_id"]){
                        echo '<button class="button" type="button" onclick="location.href=\'utente_delete.php?ID=' . $row['ID'] . '\'">Elimina</button>';
                    }
                    echo '<button class="button" type="button" onclick="location.href=\'utente_edit.php?ID=' . $row['ID'] . '\'">Modifica</button>';
                    echo '</div>';
                }
            } else {
                echo '<p class="nessun-risultato">Nessuna donna trovata nel database</p>';
